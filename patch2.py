...
with open("backup_main.go", "r") as f:
    text = f.read()
...
// ExtractLatestSignalingState returning the most recent active offer and its bundled candidates.
...
if "ParseFrontendCandidate" not in text:
    text = text.replace("func main() {", funcs + "\nfunc main() {")

old_loop_start = text.find('for _, m := range msgs {')
old_loop_end = text.find('time.Sleep(1500 * time.Millisecond)', old_loop_start)

old_loop = text[old_loop_start:old_loop_end]
...
log.Printf("ERROR: Failed parsing SDP: %v | Raw=%s", err, string(sdpRaw))
}
}
...
text = text.replace(ice_old, ice_new)
